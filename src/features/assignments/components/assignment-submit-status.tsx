...
export const AssignmentSubmitStatus = ({
  ...
  const badge = snapshot
    ...
    : { label: "No submissions yet", variant: "outline" as const };
  ...
  return (
    <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <header className="space-y-2">
        ...
        <h2 className="text-xl font-semibold text-slate-900">Submission status</h2>
      </header>
      ...
      <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-4">
        <h3 className="text-sm font-semibold text-slate-900">Due information</h3>
        <dl className="space-y-2 text-sm text-slate-700">
          <div>
            <dt className="text-xs uppercase tracking-wide text-slate-500">Due at</dt>
            <dd className="font-medium text-slate-900">{formatDueDateTime(dueAt)}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-wide text-slate-500">Time remaining</dt>
      ...
      <article className="space-y-2 rounded-lg border border-slate-200 bg-white p-4">
        ...
        {submittedAt ? (
          <p className="text-xs text-slate-500">Submitted at · {submittedAt}</p>
        ) : null}
        {snapshot?.late ? (
